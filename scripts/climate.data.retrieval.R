...
####  Getting new climate data ####
# need access to raw files from Flint
# the raw Flint data files from before 2011 are stored on the desktop in the Gremer lab
# more recent raw data files are stored on Box: https://ucdavis.app.box.com/folder/146231959671
# 2011-2016: cwd,tmx,tmn,ppt,pck (2011-2015)

# follows Megan's code from GitHub
# https://github.com/meganbontrager/climate-data-retrieval/blob/master/README.md

# will need to make a locs file with lat/long
locs=read.csv("./Raw.Data/pop.lat.long.2.csv") %>%
  dplyr::select(id = Pop, latitude = Lat, longitude = Long) %>% 
...
cwd = nc_open("~/Library/CloudStorage/Box-Box/StreptanthusDimensions/FlintBCM/flint_bcm_recent/cwd_wy2015.nc")
...
# If points are outside the range of BCM data, will still be assigned nearest point! 
# Check how far out points are falling and filter out those outside the coverage range. 
...
file_list = list.files(path = "~/Library/CloudStorage/Box-Box/StreptanthusDimensions/FlintBCM/flint_bcm_recent", full.names = TRUE)
...
for (j in 1:length(file_list)){
  ...
  file_month = 1:12
  # Month 1 = October of previous year! ## IMPORTANT ##
...
all_data = climate_list %>% 
  ...
  mutate(month = as.numeric(month),
         clim_month = if_else(month <= 3, month + 9, month - 3), # adjusting from wy dates to regular dates
         clim_year = ifelse(month <= 3, year - 1, year), # adjusting from wy dates to regular dates
         month_pad = str_pad(clim_month, 2, pad = "0"), 
         clim_date = as.numeric(paste0(clim_year, month_pad))) %>% 
  dplyr::select(-month_pad) %>% 
  dplyr::rename(tmin = tmn, tmax = tmx, ppt_mm = ppt) %>% # change variables to what is needed
  dplyr::select(id, clim_year, clim_month, clim_date, everything())

write.csv(all_data, "new.Flint.sp.pops.csv")
